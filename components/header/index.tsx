...
const Navigation = () => {
    const [lang, setIsBangla] = useState<boolean>(true);
    const router = useRouter();

    const handleInternationalize = () => {
        setIsBangla(!lang)
        router.replace(`/${lang ? 'bn' : 'en'}/product`);

    }

    return (
        <div className="sticky top-0 z-50 md:h-[65px] border-b bg-white">
            <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-4 md:px-7 py-3">
                <div className="flex gap-2">
                    <button className="xl:hidden " name="menu-toggle" type="button">
                        <svg
                            stroke="currentColor"
                            fill="currentColor"
                            strokeWidth="0"
                ...
                <div className="flex items-center space-x-4 md:space-x-6">
                    <div className="border-1 flex  max-h-96 items-center gap-3 rounded-md md:gap-6">
                        ...
                        <button onClick={handleInternationalize} className="cursor-pointer items-center gap-1 rounded border px-2 py-[2px] hover:bg-slate-50 md:flex">
                            <svg
